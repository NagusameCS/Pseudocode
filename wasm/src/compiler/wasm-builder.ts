 ...
 */
export function encodeSLEB128BigInt(value: bigint): number[] {
    const bytes: number[] = [];
    let more = true;
...
export class WasmBuilder {
    ...
    private imports: { module: string; name: string; kind: number; typeIdx: number }[] = [];
    private functions: number[] = [];     // Type indices for functions
    ...
    private codes: number[][] = [];       // Function bodies
    private data: { offset: number; bytes: number[] }[] = [];
    
    private typeMap: Map<string, number> = new Map();
    private importFuncCount: number = 0;
 ...
 */
export function createWasmBuilder(): WasmBuilder {
