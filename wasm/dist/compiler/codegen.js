 ...
 */
import { createWasmBuilder, TYPE_I32, TYPE_I64, TYPE_F64, OP, encodeULEB128, encodeSLEB128 } from './wasm-builder';
 ...
 */
function registerFunction(state, func) {
 ...
 */
function compileTopLevel(state, statements) {
 ...
 * Compile a function.
 */
function compileFunction(state, func) {
    const ctx = {
        name: func.name,
        params: func.params,
        locals: [],
 ...
 * Compile a statement.
 */
function compileStatement(state, stmt) {
 ...
 */
function compileExpression(state, expr) {
    const ctx = state.currentFunc;
    switch (expr.kind) {
        case 'Literal':
            compileLiteral(state, expr);
            break;
        case 'Identifier': {
        ...
        case 'Binary':
            compileBinary(state, expr);
            break;
        case 'Unary':
            compileUnary(state, expr);
            break;
        case 'Logical':
            compileLogical(state, expr);
            break;
        case 'Call':
        ...
        case 'Array':
            ...
            break;
        case 'Index':
            compileIndex(state, expr);
            break;
        case 'Assign':
            compileAssignment(state, expr.target, expr.value);
 ...
 */
function compileLiteral(state, lit) {
    const ctx = state.currentFunc;
    switch (lit.literalType) {
        case 'nil':
            ...
            break;
        case 'bool':
            emitBool(ctx, lit.value);
            break;
        case 'int':
            emitInt(ctx, lit.value);
            break;
        case 'float':
            emitFloat(ctx, lit.value);
            break;
        case 'string':
            emitString(state, lit.value);
            break;
    }
}
/**
 ...
 */
function compileBinary(state, expr) {
    const ctx = state.currentFunc;
    // Compile operands
 ...
 */
function compileUnary(state, expr) {
    const ctx = state.currentFunc;
 ...
 */
function compileLogical(state, expr) {
    const ctx = state.currentFunc;
    compileExpression(state, expr.left);
    // Check truthiness
 ...
 */
function compileIndex(state, expr) {
    const ctx = state.currentFunc;
    // Get array pointer
 ...
 */
function compileAssignment(state, target, value) {
    const ctx = state.currentFunc;
 ...
 */
function compileForRange(state, stmt) {
    const ctx = state.currentFunc;
 ...
 * Emit string value.
 */
function emitString(state, value) {
    const ctx = state.currentFunc;
