...
/* Create a new upvalue pointing to a stack slot */
ObjUpvalue *new_upvalue(VM *vm, Value *slot)
{
    ObjUpvalue *upvalue = (ObjUpvalue *)allocate_object(vm, sizeof(ObjUpvalue), OBJ_UPVALUE);
...
void free_object(VM *vm, Obj *object)
{
    ...
    case OBJ_GENERATOR:
    {
        ...
        FREE(vm, ObjGenerator, object);
        break;
    }
    case OBJ_PROMISE:
    {
        FREE(vm, ObjPromise, object);
        break;
    }
    case OBJ_MODULE:
    {
        /* exports dict is managed separately */
