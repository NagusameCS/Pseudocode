...
ObjTensor* tensor_randn(VM* vm, uint32_t ndim, const uint32_t* shape) {
    ...
    for (uint32_t i = 0; i < tensor->size; i += 2) {
        ...
        tensor->data[i] = z0;
        if (i + 1 < tensor->size) tensor->data[i + 1] = z1;
    }
    return tensor;
}
...
ObjTensor* tensor_from_array(VM* vm, ObjArray* arr) {
    ...
    for (uint32_t i = 0; i < arr->count; i++) {
        ...
        if (IS_NUM(v)) tensor->data[i] = as_num(v);
        else if (IS_INT(v)) tensor->data[i] = (double)as_int(v);
        else tensor->data[i] = 0.0;
    }
    return tensor;
}
...
ObjMatrix* matrix_from_array(VM* vm, ObjArray* arr) {
    ...
    if (arr->count == 0) return matrix_zeros(vm, 0, 0);
    ...
    for (uint32_t i = 0; i < rows; i++) {
        if (!IS_ARRAY(arr->values[i])) continue;
        ObjArray* row = AS_ARRAY(arr->values[i]);
        for (uint32_t j = 0; j < cols && j < row->count; j++) {
            Value v = row->values[j];
            if (IS_NUM(v)) mat->data[i * cols + j] = as_num(v);
            else if (IS_INT(v)) mat->data[i * cols + j] = (double)as_int(v);
            else mat->data[i * cols + j] = 0.0;
        }
    }
...
ObjMatrix* matrix_inverse(VM* vm, ObjMatrix* a) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (fabs(pivot) < 1e-10) return NULL;
...
double tensor_mse_loss(ObjTensor* pred, ObjTensor* target) {
    if (pred->size != target->size) return -1.0;
    double sum = 0.0;
...
ObjDataFrame* dataframe_from_dict(VM* vm, ObjDict* dict) {
    if (dict->count == 0) return dataframe_create(vm, 0);
        ...
        switch (entry->op) {
            case GRAD_OP_ADD: {
                ...
                if (in1->requires_grad) {
                    if (!in1->grad) in1->grad = tensor_zeros(vm, in1->ndim, in1->shape);
                    tensor_add(in1->grad->data, in1->grad->data, out_grad->data, in1->size);
                }
                if (in2->requires_grad) {
                    if (!in2->grad) in2->grad = tensor_zeros(vm, in2->ndim, in2->shape);
